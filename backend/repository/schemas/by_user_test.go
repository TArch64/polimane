...
func TestByUser(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		mock.ExpectQuery(`SELECT "schemas"\."id","schemas"\."created_at","schemas"\."updated_at","schemas"\."name","schemas"\."palette","schemas"\."size","schemas"\."beads","schemas"\."screenshoted_at" FROM "schemas" JOIN user_schemas ON user_schemas\.schema_id = schemas\.id AND user_schemas\.user_id = \$1`).
			WithArgs(user.ID).
			WillReturnRows(sqlmock.NewRows([]string{"id", "created_at", "updated_at", "name", "palette", "size", "beads", "screenshoted_at"}).
