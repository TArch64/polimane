...
func (s *Service) Downgrade(ctx context.Context, options *ChangeOptions) (err error) {
	from := options.User.Subscription.Plan()

	if err = s.canDowngrade(from, options.Plan); err != nil {
		return err
	}
...
func (s *Service) canDowngrade(from, to *model.SubscriptionPlan) error {
